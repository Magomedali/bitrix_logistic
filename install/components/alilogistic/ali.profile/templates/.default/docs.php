	...
	<div class="col-md-12 docs">
		<?php 
			if($contract_nds && isset($contract_nds['VALUE']) && file_exists($file_path.$contract_nds['VALUE'])){
				?>
				<div>
					<?php echo Html::a("Договор с ндс",$component->getActionUrl("docs",['d'=>$contract_nds['ID']]),['target'=>"_blank"]); ?>
				</div>
			<?php }
			...
			if($contract_nonds && isset($contract_nonds['VALUE']) && file_exists($file_path.$contract_nonds['VALUE'])){
				?>
				<div>
					<?php echo Html::a("Договор без ндс",$component->getActionUrl("docs",['d'=>$contract_nonds['ID']]),['target'=>"_blank"]); ?>
				</div>
			<?php 
			}						
		?>
