					...
					<div class="col-xs-5">
						<div class="geocoder geocoder-region">
							<p>
								<label>Регион</label>
								<?php
									echo Html::input("text","ROUTES_END[REGION]",$route && isset($route['REGION']) ? $route['REGION'] : null,['class'=>'form-control region','autocomplete'=>"off"]);
								?>
							</p>
						</div>
						<div class="geocoder geocoder-address">
							<p>
								...
								<?php
									echo Html::input("text","ROUTES_END[ADDRESS]",$route && isset($route['ADDRESS']) ? $route['ADDRESS'] : null,['class'=>'form-control address','autocomplete'=>"off"]);
								?>
							</p>
						</div>
						<p>
							<label>Дата с:</label>
							<?php
								echo Html::input("datetime-local","ROUTES_END[START_AT]",$route && isset($route['START_AT']) ? date("Y-m-d\TH:i",strtotime($route['START_AT'])) : date("Y-m-d\TH:i",time()),['class'=>'form-control startdate','max'=>"9999-12-31T23:59"]);
							?>
						...
						<div class="autocomplete autocomplete-org">
							<p>
								...
								<?php
									echo Html::input("text","ROUTES_END[ORGANISATION]",$route && isset($route['ORGANISATION']) ? $route['ORGANISATION'] : null,['class'=>'form-control org','autocomplete'=>'off']);
								?>
							</p>
						</div>
						<div class="autocomplete autocomplete-person">
							<p>
								...
								<?php
									echo Html::input("text","ROUTES_END[PERSON]",$route && isset($route['PERSON']) ? $route['PERSON'] : null,['class'=>'form-control person','autocomplete'=>'off']);
								?>
							</p>
						</div>
					</div>
					<div class="col-xs-5">
						...
						<div class="geocoder geocoder-town">
							<p>
								...
								<?php
									echo Html::input("text","ROUTES_END[TOWN]",$route && isset($route['TOWN']) ? $route['TOWN'] : null,['class'=>'form-control town','autocomplete'=>"off"]);
								?>
							</p>
						</div>
						...
						<p>
							<label>Дата по:</label>
							<?php
								echo Html::input("datetime-local","ROUTES_END[FINISH_AT]",$route && isset($route['FINISH_AT']) ? date("Y-m-d\TH:i",strtotime($route['FINISH_AT'])) : date("Y-m-d\TH:i",time()),['class'=>'form-control finishdate','max'=>"9999-12-31T23:59"]);
							?>
							<span class="dt_error"></span>
						</p>
						<p>
